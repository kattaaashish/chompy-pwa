...
- **Marketing video → `marketing/video/`** (isolated Remotion project, own
  `package.json`; not part of the app build). See its README + **Marketing video**
  section below. Screens are real app screenshots captured by `tools/capture.mjs`
  from a seeded demo account. Audio-gen scripts live in `scripts/`; setup guides
  (PDF/HTML) in `docs/`.
...
30s vertical ad (1080×1920 @ 30fps, composition `ChompyAd`), built in **Remotion**.
Self-contained: its own `package.json`/`node_modules`, never part of the app build.
- **Run:** `cd marketing/video && npm i` → `npm run studio` (preview) / `npm run
  render` (→ `out/chompy-ad.mp4`). Rendering works despite a macOS<15 warning.
- **Real UI:** `tools/seed.mjs` refreshes a demo account on the live app, then
  `tools/capture.mjs` (Playwright) screenshots the screens into `public/shots/`
  (+ `layout.json`); scenes animate crops of those, so the UI is pixel-accurate.
- **Audio (ElevenLabs):** generated by root `scripts/generate-voice.mjs` (v3 TTS,
  5 VO clips `vo-1-snap`…`vo-5-logo`) and `scripts/generate-music.mjs` (Music API
  bed). Creds come from a repo-root `.env` (`ELEVENLABS_API_KEY`,
  `ELEVENLABS_VOICE_ID`, both gitignored): `node --env-file=.env scripts/…`. Files
  land in `marketing/video/public/audio/` and are **gitignored** (`*.mp3`/`*.mp4`
  — regenerate, don't commit). `tools/audio-manifest.mjs` (a pre-hook) wires
  whatever's present; music is mixed at 0.20 with a 1s/2s fade via the `<Audio
  volume={fn}>` in `src/ChompyAd.tsx` (Remotion's bundled ffmpeg lacks `afade`, so
  fades are done in Remotion, not ffmpeg).
- **Published:** the rendered MP4 lives on a **separate public R2 bucket
  `chompy-marketing`** (not the private `chompy-meal-photos`; no Worker binding) at
  `https://pub-c2be4fe9018b485684e4a9b5c17afdc3.r2.dev/chompy-ad.mp4`. README
  embeds it. Re-upload with `wrangler r2 object put chompy-marketing/chompy-ad.mp4
  --file marketing/video/out/chompy-ad.mp4 --content-type video/mp4 --remote`.
- **Setup docs:** `docs/chompy-setup.pdf` (+ `.html` source) generated by
  `marketing/video/tools/make-pdf.mjs` (Playwright print-to-PDF). The PDF is a
  handout and *does* print the master OTP; the README deliberately does not.
